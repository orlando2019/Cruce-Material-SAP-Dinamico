...
st.set_page_config(page_title="Sistema de Gestión de Usuarios", layout="wide")
...
class UserAuth:
    ...
    def register_user(self, username, password, email="", name="", role="user", enabled=True):
    ...
    def logout(self):
        ...
        session_keys = ['username', 'email', 'name', 'authenticated', 'last_activity_time', 'auth_time', 'role']
...
def login_page():
    ...
    with col2:
        # Título y logo
        st.markdown("<h1 style='text-align: center;'>Sistema de Gestión de Usuarios</h1>", unsafe_allow_html=True)
        
        # Descripción
        st.markdown("<p style='text-align: center; padding: 20px;'>Inicie sesión para acceder al sistema.</p>", unsafe_allow_html=True)
        ...
        # Pie de página
        st.markdown("<p style='text-align: center; margin-top: 40px; color: #888;'>© 2025 - Sistema de Gestión de Usuarios</p>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if not users:
            ...
            with col2:
                ...
                user_to_edit = st.selectbox("Seleccionar Usuario", options=list(users.keys()))
            ...
            if user_to_edit:
                with st.form("edit_user_form"):
                    new_username = st.text_input("Nombre de Usuario", value=user_to_edit)
                    edit_name = st.text_input("Nombre Completo", value=user_data.get("name", ""))
                    edit_email = st.text_input("Email", value=user_data.get("email", ""))
                    edit_role = st.selectbox("Rol", options=["user", "admin"], index=0 if user_data.get("role", "") == "user" else 1)
                    edit_enabled = st.checkbox("Usuario Habilitado", value=user_data.get("enabled", True))
                    change_pwd = st.checkbox("Cambiar Contraseña")
                    new_pwd = st.text_input("Nueva Contraseña", type="password", disabled=not change_pwd)
                    ...
                    if submit_edit:
                        ...
                        if username_changed and new_username in users:
                            st.error(f"El nombre de usuario '{new_username}' ya existe. Elija otro.")
                        else:
                            ...
                            if username_changed:
                                ...
                                success, message = True, f"Usuario actualizado de '{user_to_edit}' a '{new_username}'"
                            else:
                                # Actualizar el usuario existente
                                success, message = auth.update_user(user_to_edit, update_data)
                            
                            # Si se solicitó cambio de contraseña
                            if change_pwd and new_pwd:
                                # Usar el nuevo nombre de usuario si cambió
                                target_user = new_username if username_changed else user_to_edit
                                pwd_success, pwd_message = auth.change_password(target_user, new_pwd)
                                if pwd_success:
...
def main_app():
    ...
    st.write("Esta es la aplicación principal a la que solo pueden acceder usuarios autenticados.")
    st.write("Aquí puedes desarrollar la funcionalidad principal de tu aplicación.")
    
    # Ejemplo de contenido
    st.info("Esta es una aplicación de ejemplo para demostrar el sistema de autenticación y gestión de usuarios.")
